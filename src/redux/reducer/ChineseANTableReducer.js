...
import ChineseANTbApi from '../../network/api/ChineseANTbApi'
import {filterActions} from './index'
const initialState = new antTableInitialState()
    .setIn(['apiName'], ChineseANTbApi.apiName)
    .setIn(['scroll'], {
        x: 13000,//越大,tb的内容就越宽,就能一行显示下更多内容,每行的内容就不会因显示不下而换行显示
        // y: 600 //y赋值会导致 表头的内容和 其列的内容不对齐
    });


export default function ChineseANTableReducer(state = initialState, action) {
